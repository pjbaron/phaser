 ...
 *
 */
BeamSurface.prototype.create = function(_width, _height)
{
	...
	console.log("BeamSurface.create " + this.cellSourceSize[0].wide +  "x" + this.cellSourceSize[0].high + " isNPOT = " + (this.isNPOT ? "true" : "false"));
	...
	this.cellTextureBounds[0] = new BeamRectangle(0, 0, 1, 1);
};
 ...
 */
BeamSurface.prototype.createSingle = function(_imageData, _rttTexture, _rttTextureRegister, _trimmedFrom, _offsets)
{
	...
	console.log("BeamSurface.createSingle " + this.cellSourceSize[0].wide +  "x" + this.cellSourceSize[0].high + " isNPOT = " + (this.isNPOT ? "true" : "false"));
...
BeamSurface.prototype.createGrid = function(_wide, _high, _numWide, _numHigh, _imageData, _rttTexture, _rttTextureRegister, _trimmedFrom, _offsets)
{
	...
	console.log("BeamSurface.createGrid " + srcWide +  "x" + srcHigh + " " + this.cellsWide + "x" + this.cellsHigh + " isNPOT = " + (this.isNPOT ? "true" : "false"));
	...
	for(var y = 0; y < this.cellsHigh; y++)
	{
		for(var x = 0; x < this.cellsWide; x++)
		{
			...
			this.cellTextureBounds[i++] = new BeamRectangle(x * texWide, y * texHigh, texWide, texHigh);
			if (_offsets !== undefined)
			{
 ...
 * @param  {[type]} _imageData          [description]
 */
BeamSurface.prototype.createAtlas = function(data, _imageData)
{
	var w = data.meta.size.w;
	var h = data.meta.size.h;
	this.isNPOT = !(this.isPowerOfTwo(w) && this.isPowerOfTwo(h));

	console.log("BeamSurface.createAtlas " + w + "x" + h + " frames = " + data.frames.length + " isNPOT = " + (this.isNPOT ? "true" : "false"));
	...
	for(var i = 0, l = this.cells; i < l; i++)
	{
		...
		// the percentage of the source texture occupied by each animation cell
		this.cellTextureBounds[i] = new BeamRectangle(f.frame.x / w, f.frame.y / h, f.frame.w / w, f.frame.h / h);
		// if (f.trimmed)
		// {
		// 	if (!this.cellOffsets)
		// 	{
...
BeamSurface.prototype.setCellFrame = function( _cellFrame, _x, _y, _width, _height )
{
	...
	this.cellTextureBounds[ _cellFrame ] = new BeamRectangle(_x / w, _y / h, _width / w, _height / h);
 ...
 * @param  {[type]} _imageData          [description]
 */
BeamSurface.prototype.createStarlingAtlas = function(data, _imageData)
{
	...
	console.log("BeamSurface.createAtlas " + w + "x" + h + " frames = " + d.childElementCount + " isNPOT = " + (this.isNPOT ? "true" : "false"));
	...
	for(var i = 0, l = this.cells; i < l; i++)
	{
		...
		// the percentage of the source texture occupied by each animation cell
		this.cellTextureBounds[i] = new BeamRectangle(xi / w, yi / h, wi / w, hi / h);
 ...
 * @param  {[type]} _imageData          [description]
 */
BeamSurface.prototype.createAtlasFromJSON = function(_JSON, _imageData)
{
...
BeamSurface.prototype.destroy = function()
{
	this.imageData = null;
	this.cellSourceSize = null;
	this.cellTextureBounds = null;
...
BeamSurface.prototype.isPowerOfTwo = function(x)
{
	return ((x > 0) && !(x & (x - 1)));
};


BeamSurface.prototype.dirty = function()
{
	this._isDirty = true;
};
