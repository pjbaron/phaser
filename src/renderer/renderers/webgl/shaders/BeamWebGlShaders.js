/**
 *
 * BeamWebGlShaders.js - data and support code for webGl shaders
 * 
 */
...
// static variables
BeamWebGlShaders.currentProgram = -1;



function BeamWebGlShaders()
{
...
BeamWebGlShaders.prototype.create = function()
{
...
BeamWebGlShaders.prototype.destroy = function()
{
	this.clearProgram();
...
BeamWebGlShaders.prototype.addJSON = function( jsonString )
{
...
BeamWebGlShaders.prototype.add = function( _shaderSource )
{
...
BeamWebGlShaders.prototype._getShader = function( sources, typeString )
{
...
BeamWebGlShaders.prototype.createProgram = function( _source )
{
	console.log( "BeamWebGlShaders.createProgram" );
	...
	if ( !gl.getProgramParameter( program, gl.LINK_STATUS ) )
	{
		...
		console.log( "BeamWebGlShaders.createProgram ERROR: ", gl.getProgramInfoLog( program ), "\n", _source );
		...
		for(var a in _source.attributes)
		{
			if (_source.attributes.hasOwnProperty(a))
			{
				...
				if (program.attributes[attribute] === null)
					console.log("WARNING (BeamWebGlShaders.createProgram): filter attribute returned NULL for", attribute, "it's probably unused in the filter", _source);
			}
		}
	}
		...
		for(var u in _source.uniforms)
		{
			if (_source.uniforms.hasOwnProperty(u))
			{
				...
				if (program.uniforms[uniform] === null)
					console.log("WARNING (BeamWebGlShaders.createProgram): filter uniform returned NULL for", uniform, "it's probably unused in the filter", _source);
			}
		}
	}
		...
		for(var s in _source.samplers)
		{
			if (_source.samplers.hasOwnProperty(s))
			{
				...
				if (program.samplerUniforms[sampler] === null)
					console.log("WARNING (BeamWebGlShaders.createProgram): filter sampler returned NULL for", sampler, "it's probably unused in the filter", _source);
			}
		}
	}
 ...
 */
BeamWebGlShaders.prototype.setProgram = function(_programIndex, _textureNumber)
{
	if (BeamWebGlShaders.currentProgram != _programIndex)
	{
		// remove the old program
		this.clearProgram();
		
		// set the new program
		BeamWebGlShaders.currentProgram = _programIndex;
		var program = this.programList[ BeamWebGlShaders.currentProgram ];
 ...
 *
 */
BeamWebGlShaders.prototype.clearProgram = function()
{
	if (BeamWebGlShaders.currentProgram != -1)
	{
		var program = this.programList[ BeamWebGlShaders.currentProgram ];
...
BeamWebGlShaders.prototype.prepare = function(_textureNumber)
{
	...
	if (program.uniforms)
	{
		...
		if (program.uniforms.uProjectionMatrix)
			gl.uniformMatrix3fv( program.uniforms.uProjectionMatrix, false, BeamMatrix3.makeProjection(gl.drawingBufferWidth, gl.drawingBufferHeight) );

		// set up a 3D projection matrix in the vertex shader
		if (program.uniforms.uProjectionMatrix4)
			gl.uniformMatrix4fv( program.uniforms.uProjectionMatrix4, false, BeamMatrix4.makeProjection(gl.drawingBufferWidth, gl.drawingBufferHeight) );
	}
};


BeamWebGlShaders.prototype.getAttribute = function( nameString )
{
	var program = this.programList[ BeamWebGlShaders.currentProgram ];
...
BeamWebGlShaders.prototype.getUniform = function( nameString )
{
	var program = this.programList[ BeamWebGlShaders.currentProgram ];
...
BeamWebGlShaders.prototype.getSampler = function( nameString )
{
...
BeamWebGlShaders.prototype.getCurrentProgram = function()
{
	return this.programList[ BeamWebGlShaders.currentProgram ];
};
