 ...
 * pbWebGlDrawBatch
 *
 * Contains methods for BeamWebGl which draw a batch of ImageData or BeamSurface sprites to the webGl display buffer
 *
...
// batch images, one full surface cell only, no transforms, BeamSimpleLayer, pbBunnyDemo
// requires _list to be alternately x and y coordinate values
BeamWebGl.prototype.blitSimpleDrawImages = function( _list, _listLength, _surface, _textureNumber )
{
...
// batch images with animation cells on the surface, no transforms, BeamSimpleLayer, pbBunnyDemo
...
BeamWebGl.prototype.blitSimpleDrawAnimImages = function( _list, _listLength, _surface, _textureNumber )
{
	var prepFlag = this.shaders.setProgram(this.shaders.blitShaderProgram, _textureNumber);
	prepFlag |= this.textures.prepare( _surface.imageData, null, _surface.isNPOT );
	if (prepFlag)
	...
	// NOTE: assumes that all cell sources on a BeamSimpleLayer surface are equally sized
	var scale = 1.0;
	var wide = _surface.cellSourceSize[0].wide * scale * 0.5 / screenWide2;
	var high = _surface.cellSourceSize[0].high * scale * 0.5 / screenHigh2;
	...
	// NOTE: assumes that all cell textures on a BeamSimpleLayer surface are equally sized
...
BeamWebGl.prototype.blitListDirect = function( _list, _listLength, _surface, _textureNumber )
{
	var prepFlag = this.shaders.setProgram(this.shaders.blitShaderProgram, _textureNumber);
...
// currently unused in demos.  pbImage.isParticle through BeamWebGlLayer, sends four floats per vertex (x,y,u,v) to gl, no sprite sheet
// TODO: don't need u,v stream if it's always 0 & 1 values??
BeamWebGl.prototype.blitDrawImages = function( _textureNumber, _list, _surface )
{
	var prepFlag = this.shaders.setProgram(this.shaders.blitShaderProgram, _textureNumber);
	prepFlag |= this.textures.prepare( _surface.imageData, null, _surface.isNPOT );
	if (prepFlag)
...
BeamWebGl.prototype.blitDrawImagesPoint = function( _list, _listLength, _surface, _textureNumber )
{
...
BeamWebGl.prototype.blitDrawImagesPointAnim = function(_list, _listLength, _surface, _textureNumber )
{
...
BeamWebGl.prototype.batchDrawImages = function( _textureNumber, _list, _surface )
{
...
// _list object format: { image: pbImage, transform: BeamMatrix3, z_order: Number }
BeamWebGl.prototype.rawBatchDrawImages = function( _textureNumber, _list )
{
...
// Sends transform matrix elements to gl.
// _list object format: { image: pbImage, transform: BeamMatrix3, z_order: Number }
BeamWebGl.prototype.rawBatchDrawTextures = function( _list )
{
	var surface = _list[0].image.surface;
