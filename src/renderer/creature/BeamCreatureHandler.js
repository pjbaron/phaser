...
function BeamCreatureHandler(_renderer, _shaderProgram)
{
...
BeamCreatureHandler.prototype.constructor = BeamCreatureHandler;
 ...
 */
BeamCreatureHandler.prototype.create = function(name, json, srcTexture, srcTextureRegister, dstTextureRegister, dstWidth, dstHeight, transform, animSpeed)
{
	console.log("BeamCreatureHandler.Create " + name);

	if (!this.dictionary)
	{
		this.dictionary = new BeamDictionary();
	...
	var rttTexture = BeamWebGlTextures.initTexture(dstTextureRegister, dstWidth, dstHeight);
	var rttRenderbuffer = BeamWebGlTextures.initDepth(rttTexture);
	var rttFramebuffer = BeamWebGlTextures.initFramebuffer(rttTexture);
...
BeamCreatureHandler.prototype.destroy = function()
{
	if (this.dictionary)
 ...
 */
BeamCreatureHandler.prototype.add = function(name, x, y, r, scale, speed)
{
	if (this.dictionary.exists(name))
	{
 ...
 */
// BeamCreatureHandler.prototype.bounds = function(_typeName, _animation, _timeStep)
// {
...
BeamCreatureHandler.prototype.adjust = function(_name, _transform)
{
 ...
 */
BeamCreatureHandler.prototype.update = function(timeInterval)
{
...
BeamCreatureHandler.prototype.updateType = function(creatures)
{
	// there's only one dictionary entry per type
	...
	creatureData.renderer.DrawCreature(creatureData.transform, BeamPhaserRender.renderer.graphics, this.shaderProgram, creatureData.srcTextureRegister);
 ...
 */
BeamCreatureHandler.prototype.getAll = function()
{
	var creatureList = [];
