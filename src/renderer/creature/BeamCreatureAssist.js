...
function BeamCreatureAssist( docId )
{
	console.log( "BeamCreatureAssist c'tor entry" );

	this.phaserRender = new BeamPhaserRender( docId );
	this.phaserRender.create( 'webgl', this.create, this.update, this );
	// font for User Interface
	BeamPhaserRender.loader.loadImage( "font", "../img/fonts/arcadeFonts/8x8/Battle Bakraid (Eighting).png", 8, 8, 95, 5 );

	// TODO: change these values for your Creature

	// the shader we'll use to draw the Creature (usually "stripShaderSources.json")
	this.creatureShaderJSON = BeamPhaserRender.loader.loadFile( "../json/stripShaderSources.json" );
	// the creature animation data in a .zip file (just zip the .json data output by the Creature editor)
//	this.creatureDataZip = BeamPhaserRender.loader.loadFile( "../img/creatures/utah.CreaExport/character_data.zip", "arraybuffer" );
	this.creatureDataZip = BeamPhaserRender.loader.loadFile( "../img/creatures/coel.CreaExport/character_data.zip", "arraybuffer" );
	// the creature source image (usually a 32 bit .png file)
//	BeamPhaserRender.loader.loadImage( "creature", "../img/creatures/utah.CreaExport/character_img.png" );
	BeamPhaserRender.loader.loadImage( "creature", "../img/creatures/coel.CreaExport/character_img.png" );
...
BeamCreatureAssist.prototype.create = function()
{
	console.log("BeamCreatureAssist.create");

	// get the shader program into the shaders system
	var jsonString = BeamPhaserRender.loader.getFile( this.creatureShaderJSON ).responseText;
	this.creatureShaderProgram = BeamPhaserRender.renderer.graphics.shaders.addJSON( jsonString );
	...
	var zip = new JSZip( BeamPhaserRender.loader.getFile( this.creatureDataZip ).response );
	var creatureJSON = zip.file("character_data.json").asText();
	var creatureData = JSON.parse(creatureJSON);

	// make a creature handler
	this.creatures = new BeamCreatureHandler(this.phaserRender, this.creatureShaderProgram);
	...
	this.typeTransform = BeamMatrix3.makeTransform(this.xpos, this.ypos, this.rot, this.scale, this.scale);
	...
	this.creatures.add(
			...
			BeamPhaserRender.width * 0.5, 	// x
			BeamPhaserRender.height * 0.5,	// y
			0, 							// rotation
			1.0,						// scale
			0.0							// speed
		);
	...
	BeamPhaserRender.renderer.postUpdate = this.postUpdate;
	...
	//this.uiLayer.create(BeamPhaserRender.rootLayer, this.phaserRender, 0, 0, 0, 0, 1, 1);
	// PJBNOTE: rootLayer has been replaced with game.world... we'll need a 'game' reference here
	//BeamPhaserRender.rootLayer.addChild(this.uiLayer);

	// prepare the text strings
	this.text = new BeamText();
	...
	// controls
	this.buttons = new BeamButtons();
	this.buttons.create(document.body);

	this.help1 = this.text.addLine("< x pos >", 400, 10, 8);
	this.buttons.add("x_less", new BeamRectangle(395, 5, 10, 10), this.pressButton, this);
	this.buttons.add("x_more", new BeamRectangle(455, 5, 10, 10), this.pressButton, this);

	this.help2 = this.text.addLine("< y pos >", 400, 20, 8);
	this.buttons.add("y_less", new BeamRectangle(395, 15, 10, 10), this.pressButton, this);
	this.buttons.add("y_more", new BeamRectangle(455, 15, 10, 10), this.pressButton, this);

	this.help3 = this.text.addLine("<  rot  >", 400, 30, 8);
	this.buttons.add("rot_less", new BeamRectangle(395, 25, 10, 10), this.pressButton, this);
	this.buttons.add("rot_more", new BeamRectangle(455, 25, 10, 10), this.pressButton, this);

	this.help4 = this.text.addLine("< scale >", 400, 40, 8);
	this.buttons.add("scale_less", new BeamRectangle(395, 35, 10, 10), this.pressButton, this);
	this.buttons.add("scale_more", new BeamRectangle(455, 35, 10, 10), this.pressButton, this);
};


BeamCreatureAssist.prototype.pressButton = function(_down, _key)
{
...
BeamCreatureAssist.prototype.destroy = function()
{
	console.log("BeamCreatureAssist.destroy");
...
BeamCreatureAssist.prototype.restart = function()
{
	console.log("BeamCreatureAssist.restart");
...
BeamCreatureAssist.prototype.update = function()
{
	...
	// render to the display from now on (BeamRenderer.update: BeamPhaserRender.rootLayer.update)
	// without this all other sprites in the scene will render to the last bound texture
	// TODO: this should be parameterised in BeamRenderer and set before the BeamPhaserRender.rootLayer.update call
	BeamWebGlTextures.cancelFramebuffer();
};


/**
 * postUpdate - called after BeamRenderer does BeamPhaserRender.rootLayer.update
 * display all of the creature sprite images by rendering to the display from their render-to-textures on the GPU
 *
 * @return {[type]} [description]
 */
BeamCreatureAssist.prototype.postUpdate = function()
{
	...
	var transform = BeamMatrix3.makeTransform(o.x, o.y, o.r, o.scale, o.scale);
	BeamPhaserRender.renderer.graphics.drawTextureWithTransform( o.type.dstTexture, transform, 1.0 );

	// debug box
//   	var wide = o.type.renderer.width * this.dstWidth * this.scale;
//   	var high = o.type.renderer.height * this.dstHeight * this.scale;
//		BeamPhaserRender.renderer.graphics.drawRect(o.x + o.type.renderer.left, o.y + o.type.renderer.bottom - high, wide, high, {r:0xff, g:0xff, b:0xff, a:0xff});

	// outline of texture box
	BeamPhaserRender.renderer.graphics.drawRect(BeamPhaserRender.width * 0.5 - this.dstWidth * 0.5, BeamPhaserRender.height * 0.5 - this.dstHeight * 0.5, this.dstWidth, this.dstHeight, {r:0xff, g:0xff, b:0xff, a:0xff});
};
